...
# 项目结构
+-------------------------------------------------------+
|  🖥️  前端层 (Frontend - Vue 3)                         |
|  (Views, Stores, Axios)                               |
+--------------------------+----------------------------+
                           | HTTP 请求 (带 Token)
                           v
+--------------------------+----------------------------+
|  🛡️  安全与配置层 (/security, /config)                |
|  ---------------------------------------------------  |
|  [JwtFilter] 拦截校验 -> [SecurityConfig] 授权        |
+--------------------------+----------------------------+
                           | 认证通过
                           v
+--------------------------+----------------------------+
|  🎮  API 接口层 (/controller)                         |
|  ---------------------------------------------------  |
|  职责：接收前端 JSON -> 映射为 [DTO] -> 参数校验      |
|  文件：AuthController, MovieController...             |
+--------------------------+----------------------------+
                           | ⬇️ 传入 DTO
                           | ⬆️ 返回 VO/DTO
+--------------------------+----------------------------+
|  ⚙️  业务逻辑层 (/service)                            |
|  ---------------------------------------------------  |
|  职责：核心逻辑, 事务控制, [DTO 转 Entity]            |
|  交互：1. 调 Repository 存库                          |
|        2. 调 MinIO 上传文件                           |
|        3. 调 Redis 读写缓存                           |
|        4. 调 RabbitMQ 发消息 -----------------------+ |
+--------------------------+--------------------------|-+
   ...
   v                ^                                 v
+-------------------+-------+      +------------------+------------------+
|  💾 数据访问层 (/repo)     |      |  🐇 消息队列 (RabbitMQ)             |
|  (JPA / ES Repository)    |      +------------------+------------------+
+--+----------------+-------+                         | 监听 (异步)
   | SQL            | DSL                             v
   v                v              +------------------+------------------+
+-------+      +---------+         |  📨 消费者层 (/consumer)            |
| MySQL |      | Elastic |         |  (RatingUpdateConsumer 等)          |
+-------+      +---------+         +------------------+------------------+
                                                      |
                                                      | 调用 Service 更新数据
                                                      +-------------------->